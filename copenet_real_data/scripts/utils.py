...
def npPerspProj_airsim(intr,points3d,cam_rottrans):
    '''
    :param scale: orthographics projection scale vector
    :param points3d: Nx3 tensor
    :param cam_rot: camera pose matrix with translation appended to rotation (3x4)
    ...
    extr_trans = -np.matmul(extr_rot,cam_rottrans[:3,3].reshape(3,1))
    points3d_local = np.matmul(points3d.reshape(-1,3),cam_rottrans[:3,:3]) + extr_trans.reshape(1,3)
    # airsim camera is facing towards x axis, change the coordinates accordingly
    points3d_local = np.transpose(np.matmul(np.array([[0,1,0],[0,0,1],[1,0,0]]),np.transpose(points3d_local)))
    points2d = np.transpose(np.matmul(intr,np.transpose(points3d_local)))
...
def get_weak_persp_cam_full_img(intr,person_root_2d,person_position_wrt_cam):
    fx = intr[0,0]
    fy = intr[1,1]
    cx = intr[0,2]
    cy = intr[1,2]
...
def transform_smpl(trans_mat,smplvertices=None,smpljoints=None, orientation=None):
    verts =  torch.bmm(trans_mat[:,:3,:3],smplvertices.permute(0,2,1)).permute(0,2,1) +\
                    trans_mat[:,:3,3].unsqueeze(1)
    if smpljoints is not None:
        joints = torch.matmul(trans_mat[:,:3,:3],smpljoints.permute(0,2,1)).permute(0,2,1) +\
                         trans_mat[:,:3,3].unsqueeze(1)
    else:
        joints = None
