...
class copenet_twoview(pl.LightningModule):
    ...
    def get_loss(self,input_batch, 
        ...
        loss_keypoints = (self.mseloss(pred_joints_2d_cam0[:,:22], 
                            gt_joints_2d_cam0[:,:22,:2])*gt_joints_2d_cam0[:,:22,2:]) + \
                        (self.mseloss(pred_joints_2d_cam1[:,:22], 
                            gt_joints_2d_cam1[:,:22,:2])*gt_joints_2d_cam1[:,:22,2:])
    ...
    def fwd_pass_and_loss(self,input_batch,is_val=False,is_test=False):
        ...
        with torch.no_grad():
            ...
            in_smpltrans0 = torch.from_numpy(np.array([0,0,10])).float().expand(batch_size, -1).type_as(bb0).clone()
            in_smpltrans1 = torch.from_numpy(np.array([0,0,10])).float().expand(batch_size, -1).type_as(bb0).clone()
            # noisy input pose
        ...
        if is_test:
            ...
            output = {'pred_vertices_cam0': pred_vertices_cam0.detach().cpu(),
                        ...
                        'pred_pose0': pred_pose0.detach().cpu(),
                        'pred_pose1': pred_pose1.detach().cpu(),
                        'pred_betas0': pred_betas0.detach().cpu(),
                        'pred_betas1': pred_betas1.detach().cpu()}
        else:
    ...
    def test_dataloader(self):
        ...
        if self.hparams.testdata.lower() == "aircapdata":
            aircap_dset = aircapData.aircapData_crop(range(4615),self.hparams.datapath)
    ...
    def add_model_specific_args(parent_parser):
        ...
        train.add_argument('--copenet_home', type=str, default="/is/ps3/nsaini/projects/copenet_real", help='copenet repo home')
        ...
        train.add_argument('--limbs2d_loss_weight', default=1.5, type=float, help='Weight of limbs 3D keypoint loss')
