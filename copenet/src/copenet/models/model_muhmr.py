...
class copenet(nn.Module):
    ...
    def __init__(self, block, layers, smpl_mean_params):
        ...
        self.fc1 = nn.Linear(512 * block.expansion + 3 + 6 + npose + 10 + npose + 10, 1024)
        self.drop1 = nn.Dropout()
        self.fc2 = nn.Linear(1024, 1024)
        self.drop2 = nn.Dropout()
        self.decpose = nn.Linear(1024, 6 + npose)
    ...
    def forward(self, x0, x1, init_cam0=None, init_cam1=None, init_theta0=None, init_theta1=None, init_shape0=None, init_shape1=None, iters = 3):
        ...
        pred_pose0, pred_betas0, pred_cam0, pred_pose1, pred_betas1, pred_cam1 = self.forward_reg(xf0, xf1,
        ...
        for it in range(int(iters)-1):
            pred_pose0, pred_betas0, pred_cam0, pred_pose1, pred_betas1, pred_cam1 = self.forward_reg(xf0, xf1,
                                                                                pred_pose0[:,:6], pred_pose1[:,:6],
                                                                                pred_pose0[:,6:], pred_pose1[:,6:],
                                                                                pred_betas0, pred_betas1,
                                                                                pred_cam0, pred_cam1)

        
        return pred_pose0, pred_betas0, pred_cam0, pred_pose1, pred_betas1, pred_cam1
    ...
    def forward_reg(self, xf0, xf1,
        ...
        xc0 = torch.cat([xf0, pred_cam0, pred_orient0, pred_art_pose0, pred_shape0, pred_art_pose1, pred_shape1],1)
        xc0 = self.fc1(xc0)
        xc0 = self.drop1(xc0)
        xc0 = self.fc2(xc0)
        xc0 = self.drop2(xc0)

        xc1 = torch.cat([xf1, pred_cam1, pred_orient1, pred_art_pose1, pred_shape1, pred_art_pose0, pred_shape0],1)
        xc1 = self.fc1(xc1)
        xc1 = self.drop1(xc1)
        xc1 = self.fc2(xc1)
        xc1 = self.drop2(xc1)

        pred_pose0 = torch.cat([pred_orient0, pred_art_pose0],1) + self.decpose(xc0)
        pred_shape0 = self.decshape(xc0) + pred_shape0
        pred_cam0 = self.deccam(xc0) + pred_cam0
        
        pred_pose1 = torch.cat([pred_orient1, pred_art_pose1],1) + self.decpose(xc1)
        pred_shape1 = self.decshape(xc1) + pred_shape1
        pred_cam1 = self.deccam(xc1) + pred_cam1

        return pred_pose0, pred_shape0, pred_cam0, pred_pose1, pred_shape1, pred_cam1
