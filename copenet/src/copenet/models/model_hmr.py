...
class copenet(nn.Module):
    ...
    def forward(self, x, init_cam=None, init_theta=None, init_shape=None, iters = 3):
        batch_size = x.shape[0]

        
        if init_theta is None:
            init_pose = self.init_pose[:,:22*6].expand(batch_size, -1)
        else:
            init_pose = init_theta
        ...
        pred_pose, pred_betas, pred_cam = self.forward_reg(xf, 
                                                            init_pose,
                                                            init_shape,
                                                            init_cam)
        ...
        return pred_rotmat, pred_betas, pred_cam
    ...
    def forward_reg(self, xf, pred_pose, pred_shape, pred_cam):
