...
class hmr(pl.LightningModule):
    ...
    def get_loss(self,input_batch, pred_camera, pred_rotmat, pred_betas, pred_output_cam, pred_joints_2d_cam):
        ...
        gt_joints_2d_cam = input_batch['smpl_joints_2d_crop0'].squeeze(1)
        
        # # # Normalize keypoints to [-1,1]
        # pred_joints_2d_cam = pred_joints_2d_cam / (self.hparams.img_res / 2.)
        # gt_joints_2d_cam = gt_joints_2d_cam.squeeze(1) / (self.hparams.img_res / 2.)
    ...
    def fwd_pass_and_loss(self,input_batch,is_test=False, is_val=False):
        ...
        pred_cam_t = torch.stack([pred_camera[:,1],
                                  pred_camera[:,2],
                                  2*self.focal_length[0]/(self.hparams.img_res * pred_camera[:,0] +1e-9)],dim=-1)
        
        pred_joints_2d_cam = perspective_projection(pred_joints,
        ...
        if is_test:
            ...
            pred_vertices,pred_joints_cam,_,_ = transform_smpl(transf_mat,
                                                pred_output_cam.vertices.squeeze(1),
                                                pred_output_cam.joints.squeeze(1))
            ...
            output = {'pred_vertices_cam': pred_vertices.detach().cpu(),
                    "pred_betas": pred_betas.detach().cpu(),
                    "pred_j3d_cam": pred_joints_cam.detach().cpu(),
                    'pred_smpltrans': pred_cam_trans0.detach().cpu(),
                    'in_smpltrans': in_cam_trans0.detach().cpu(),
                    'gt_smpltrans': input_batch["smpltrans_rel0"].detach().cpu(),
